...
void Processador::acessar_memoria(unsigned int endereco, TipoAcesso tipo) {
    std::cout << "\n[" << nome << "] Ciclo " << tempo_global 
              << ": Requisitando " << (tipo == TipoAcesso::LEITURA ? "LEITURA" : "ESCRITA") 
              << " do endereco " << endereco << std::endl;
    ...
    std::cout << "[" << nome << "] Acesso completado em " << latencia 
              << " ciclo(s). Tempo total: " << tempo_global << std::endl;
}
